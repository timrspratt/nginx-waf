...
typedef struct {
    ngx_str_t block_expression;  // Store the blocking expression
    ngx_int_t block_status;      // Store the status code to return
} ngx_http_block_config_t;

static ngx_int_t ngx_http_block_handler(ngx_http_request_t *r);
static char *ngx_http_block_expression(ngx_conf_t *cf, ngx_command_t *cmd, void *conf);
...
static ngx_command_t ngx_http_block_commands[] = {
    {
        ngx_string("block_expression"),
        NGX_HTTP_MAIN_CONF | NGX_HTTP_SRV_CONF | NGX_HTTP_LOC_CONF | NGX_CONF_TAKE2,
        ngx_http_block_expression,
        NGX_HTTP_LOC_CONF_OFFSET,
        offsetof(ngx_http_block_config_t, block_expression),
        NULL
    },
    ngx_null_command
};
...
static void *ngx_http_block_create_loc_conf(ngx_conf_t *cf) {
    ngx_http_block_config_t *conf;

    conf = ngx_pcalloc(cf->pool, sizeof(ngx_http_block_config_t));
    if (conf == NULL) {
        return NGX_CONF_ERROR;
    }

    // Default status code is 403
    conf->block_status = NGX_HTTP_FORBIDDEN;

    return conf;
}

// Parse the block_expression directive from the config
static char *ngx_http_block_expression(ngx_conf_t *cf, ngx_command_t *cmd, void *conf) {
    ngx_http_block_config_t *block_conf = conf;
    ngx_str_t *value = cf->args->elts;

    // Set the expression in the configuration structure
    block_conf->block_expression = value[1];
    ...
    if (status == NGX_ERROR || status < 100 || status > 599) {
        ngx_conf_log_error(NGX_LOG_EMERG, cf, 0, "Invalid status code in block_expression");
        return NGX_CONF_ERROR;
    }
    block_conf->block_status = status;
...
static ngx_int_t ngx_str_contains(ngx_str_t *haystack, const char *needle) {
    if (ngx_strcasestrn(haystack->data, needle, ngx_strlen(needle) - 1) != NULL) {
        return 1;  // found
    }
    return 0;  // not found
}
...
static ngx_int_t ngx_http_block_handler(ngx_http_request_t *r) {
    ...
    if (conf->block_expression.len == 0) {
        return NGX_DECLINED;
    }
    ...
    if (user_agent) {
        // Evaluate block conditions based on the block_expression (you can make this more dynamic)
        if (ngx_str_contains(user_agent, "Aero CDN") || ngx_str_contains(user_agent, "ChromeHeadless") ||
            ngx_str_contains(user_agent, "Go-http-client") || ngx_str_contains(user_agent, "Bytespider") ||
            ngx_str_contains(user_agent, "python-requests")) {
            return conf->block_status;  // Return the configured status code
        }
    }

    // Example condition based on IP address (can be configured similarly)
    if (ngx_strcmp(remote_addr.data, "192.168.1.100") == 0) {
        return conf->block_status;  // Return the configured status code
    }
